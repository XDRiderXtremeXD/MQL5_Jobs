...
#resource "\\Indicators\\Indicador_SAR_Confirmacion.ex5"
...
int OnInit()
  {
   ...
   if(MA_1)
      Handle_1=iCustom(NULL,PERIOD_CURRENT,"::Indicators\\Indicador_SAR_Confirmacion.ex5",Cruce_Deshabilitar,S1,SAR_Step,SAR_Maximun,
                       S2_,
                       Period_MACD,
                       Slow_Ema_MACD,
                       Signal_Period_MACD,
   ...
   if(MA_2)
      Handle_2=iCustom(NULL,PERIOD_CURRENT,"::Indicators\\Indicador_SAR_Confirmacion.ex5",Cruce_Deshabilitar,S1,SAR_Step,SAR_Maximun,
                       S2_,
                       Period_MACD,
                       Slow_Ema_MACD,
                       Signal_Period_MACD,
   ...
   if(Bars_!=Bars_Actual)
     {
      if((!MA_1 || (MA_1 && BarsCalculated(Handle_1)==Bars_Actual)) && (!MA_2 || (MA_2 && BarsCalculated(Handle_2)==Bars_Actual)))
        {
         if(
            (!MA_1 || (MA_1 && CopyBuffer(Handle_1,1,0,3,Arrows_1_DN)>0 && CopyBuffer(Handle_1,0,0,3,Arrows_1_UP)>0))
            &&
            (!MA_2 || (MA_2 && CopyBuffer(Handle_2,1,0,3,Arrows_2_DN)>0 && CopyBuffer(Handle_2,0,0,3,Arrows_2_UP)>0))
         )
           {
            ...
            if(TipoOperacion==Only_SELL || TipoOperacion==Both)
               if((MA_1 && Arrows_1_DN[1]!=EMPTY_VALUE) || (MA_2 && Arrows_2_DN[1]!=EMPTY_VALUE))
                 {
                  double SL=(MA_1 && Arrows_1_DN[1]!=EMPTY_VALUE)?Arrows_1_DN[1]:Arrows_2_DN[1];
                  if(SL!=EMPTY_VALUE)
                    {
                     if(Bid<SL)
                       {
            ...
            if(TipoOperacion==Only_BUY || TipoOperacion==Both)
               if((MA_1 && Arrows_1_UP[1]!=EMPTY_VALUE) || (MA_2 && Arrows_2_UP[1]!=EMPTY_VALUE))
                 {
                  double SL=(MA_1 && Arrows_1_UP[1]!=EMPTY_VALUE)?Arrows_1_UP[1]:Arrows_2_UP[1];
                  if(SL!=EMPTY_VALUE)
                    {
                     if(Ask>SL)
                       {
