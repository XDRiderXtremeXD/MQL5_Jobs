...
datetime TimeStart,PrevTimeSell=0;
...
int OnInit()
  {
   ...
   BOOM_CRASH=iCustom(Symbol(),PERIOD_CURRENT,"x4_2_Bandas_BOOM",
   ...
   ArraySetAsSeries(BOOM_CRASH_SELL,true);
...
void OnTick()
  {
   ...
   CopyBuffer(BOOM_CRASH,0,0,3,BOOM_CRASH_SELL);
   ...
   for(int i=0; i<PositionsTotal(); i++)
     {
      ...
      if(PositionGetInteger(POSITION_TYPE)==POSITION_TYPE_SELL)
        {
         if(Ask<Bollinger_1_Buffer_DN[0])
           {
   ...
   if(BOOM_CRASH_SELL[1]!=EMPTY_VALUE && Sell==0 && PrevTimeSell!=Rates[0].time)
     {
      double TP=TakeProfit==0?0:Bid-TakeProfit_;
      double SL=StopLoss==0?0:Ask+StopLoss_;
      ...
      if(LOTE>=SymbolInfoDouble(Symbol(),SYMBOL_VOLUME_MIN))
        {
         bool SellTradeBool=trade.Sell(LOTE,Symbol(),Bid,SL,TP,"Expert Final"+string(TimeStart));
         if(SellTradeBool)
           {
            PrevTimeSell=Rates[0].time;
            if(AlertPC)
