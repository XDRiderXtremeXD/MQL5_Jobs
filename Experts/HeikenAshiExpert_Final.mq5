...
input int MaxRetroceso=3;//Maximo Velas Retroceso (>=1)
input int MinRetroceso=1;//Minimo Velas Retroceso (>=1)
input int MinimoConsecutivas=5;//Minimo Velas Consecutivas Direccion (>=1)
input bool retroceso_cierre_traspaso_ema9=true;//permitir retroceso cierre traspaso ema9
...
input string S321="=================  CONF PARO DESPUES DE PERDIDA =================";//=====================================================================================
input bool parar=true;//Parar despues de perder
input int horas_paro=5;//Horas paro
...
input double BE_Porcentaje_TP=50;//BE START Porcentaje (OP-TP)
...
int OnInit()
  {
   ...
   if(MinRetroceso<1)
     {
      Alert("Minimo velas retroceso tiene que ser mayor o igual a 1");
      return INIT_FAILED;
     }
         ...
         if(CargarDatos(barsActuales))
           {
            ...
            if(!PositionSelectByTicket(TicketOperacionUnica))
               if(!parar || CondicionPerdidaDiaria())
                 {
...
bool CondicionPerdidaDiaria()
  {
   datetime from_date=TimeCurrent()-PeriodSeconds(PERIOD_H1)*(horas_paro);          // desde el principio
...
bool EvaluarReversaConsecutivas(bool InicialIsBuy)
  {
   ...
   for(int i=3; i<barsCopyIndicator-1 && barsUntil(i); i++)
     {
      ...
      if(InicialIsBuy?(!isUPCandle):(isUPCandle))
         if(!velasReversaVerificacion)
           {
            if(!(InicialIsBuy?(HA_Open[i]>MA[i]):(HA_Open[i]<MA[i])))
               if(retroceso_cierre_traspaso_ema9 || !(InicialIsBuy?(HA_Close[i]>MA[i]):(HA_Close[i]<MA[i])))
                  return false;
            velasReversa++;
           }
         else
   ...
   if(velasConsecutivas>=MinimoConsecutivas && velasReversa<=MaxRetroceso && velasReversa>=MinRetroceso)
      return true;
...
bool CondicionInicialBars(bool &IsBuy)
  {
   if(MA[1]<HA_Close[1] && MA[2]<HA_Open[2] && HA_Open[1]<HA_Close[1] && HA_Open[2]>HA_Close[2] && (retroceso_cierre_traspaso_ema9 || HA_Close[2]>MA[2]))
     {
      IsBuy=true;
      return true;
     }
