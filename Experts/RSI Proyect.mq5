...
#resource "\\Indicators\\TradingView RSI Indicator.ex5"
...
input int Porcentaje_Stop=200;// Porcentaje STOP(Balance Inicio Dia)
...
int OnInit()
  {
   ...
   Handle=iCustom(Symbol(),PERIOD_CURRENT,"::Indicators\\TradingView RSI Indicator.ex5",RSI_Period,RSI_Applied,level_Up,level_Dn);
...
void OnTick()
  {
   ...
   double PerdidaMaxima=BalanceInicial*2/100.0;
   ...
   if(Activo && BalanceInicial-PerdidaMaxima>AccountInfoDouble(ACCOUNT_BALANCE))
     {
      Activo=false;
         ...
         if((TP_C>=5 || TP==0) && (SL_C>=5 || SL==0))
           {
            ...
            Alert("ERROR TP O SL , Calculo menor que 4 puntos "+"TP: "+IntegerToString(TP_C)+" SL: "+IntegerToString(SL_C));
        }
      else
         Alert("No hay suficiente balance para poner un Lote correcto");
     }
  }
//+------------------------------------------------------------------+
//|                                                                  |
