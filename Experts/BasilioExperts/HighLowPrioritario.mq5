...
input double DistanciaCierreMACD=0.05;
      ...
      if(CopyRates(Symbol(),Periodo_Operaciones,0,copiaBarras,rates)>0)
        {
         if(BarsCalculated(macd_handle)==barras_actuales && BarsCalculated(HL_handle)==barras_actuales)
           {
            if(CopyBuffer(macd_handle,0,0,3,MACD_VALOR)>0 && CopyBuffer(macd_handle,1,0,3,MACD_COLOR)>0 && CopyBuffer(HL_handle,0,0,Barras_Evaluacion+1,HIGH)>0 && CopyBuffer(HL_handle,1,0,Barras_Evaluacion+1,LOW)>0)
              {
               bool Compras=false;
               if(CondicionSalida(Compras))
                  CerrarPosiciones(Compras);
                           ...
                           if(VerificarPerdidas())
                              if(tipo_operaciones==COMPRAS_VENTAS || (EsCompra?tipo_operaciones==SOLO_COMPRAS:tipo_operaciones==SOLO_VENTAS))
                                 if(EvaluarHorario(Hora_Inicio,Hora_Final,Minuto_Inicio,Minuto_Final,rates[0].time))
                                   {
                                    Operar_(EsCompra,ExpertComentario,volumenReal,StopLoss,0);
                                   }
...
bool CondicionEntrada(bool &EsCompra)
  {
   if(MACD_COLOR[1]==0 && MACD_COLOR[2]==1)
     {
      for(int i=1;i<Barras_Evaluacion;i++)
         if(HIGH[i]>HIGH[i+1])
           {
            EsCompra=false;
            return true;
...
//|                                                                  |
//+------------------------------------------------------------------+
bool CondicionSalida(bool &Compras)
  {
   if(MACD_VALOR[0]<DistanciaCierreMACD*-1)
     {
      Compras=true;
      return true;
     }
   if(MACD_COLOR[0]>DistanciaCierreMACD)
     {
      Compras=false;
      return true;
     }
