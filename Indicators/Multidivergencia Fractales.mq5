//+------------------------------------------------------------------+
//|                                   Multidivergencia Fractales.mq5 |
   ...
   if(Tiempo!=time[rates_total-1])
     {
      ...
      if(CopyBuffer(Handle_ZZ,0,0,rates_total,ZZ_Buffer)<=0)
         return(0);
      if(CopyBuffer(Handle_ZZ,1,0,rates_total,ZZ_Buffer_L)<=0)
         return(0);
...
//Comment(rates_total-1-IndiceStart,"  Indice ",rates_total-1-Indice);
   ...
   for(int shift = START; shift < rates_total ; shift++)
     {
      int PrefixTrend2=0;
      double ZigZag_L = ZigZagValor_L(shift,rates_total);

      if(ZigZag_L>0 &&  ZigZag_L!=EMPTY_VALUE)
        {
         Valido=false;
         for(int i=shift+1; i<rates_total; i++)
           {
            double ZigZag2_L = ZigZagValor_L(i,rates_total);
            if(ZigZag2_L>0 &&  ZigZag2_L!=EMPTY_VALUE)
              {
               Indice=i;
               Valido=true;
               break;
         ...
         if(Valido)
           {
            if(ZigZag_L<=low[shift])
              {
               if(Ver_Indicador_RSI)
                  Divergencia(1,rates_total-1-shift,0,time[shift],PrefixTrend2,RSI_BUFFER,ChartIndicatorRSI,"RSI");
               if(Ver_Indicador_MACD)
                  Divergencia(1,rates_total-1-shift,0,time[shift],PrefixTrend2,MACD_BUFFER,ChartIndicatorMACD,"MACD");
               if(Ver_Indicador_ESTOCASTICO)
                  Divergencia(1,rates_total-1-shift,0,time[shift],PrefixTrend2,ESTOCASTICO_BUFFER,ChartIndicatorESTOCASTICO,"EST");
              }
            IndiceStart2=shift+1;
           }
        }
     }
   ...
   for(int j=NumeroMuestreo*-1; j<=NumeroMuestreo; j++)
     {
      ...
      if(ZZ>0  && ZZ!=EMPTY_VALUE && ((IsHigh && ZZ>=iHigh(Symbol(),PERIOD_CURRENT,i+j)) || (!IsHigh && ZZ<=iLow(Symbol(),PERIOD_CURRENT,i+j))))
        {
         IndiceZigZag=i+j;
         return true;
        }
...
//|                                                                  |
//+------------------------------------------------------------------+
double ZigZagValor_L(int Indice_ArraySeries_true,int rates_total)
  {
   double ZZ=ZZ_Buffer_L[rates_total-1-(Indice_ArraySeries_true)];
   return ZZ;
  }
//+------------------------------------------------------------------+
...
void Estadistica(int IndiceI)
  {
   ...
   Comment("Segunda Vela= ",SegundaVela,"  Tercera Vela= ",TerceraVela,"  Cuarta Vela= ",CuartaVela,"  Quinta Vela= ",QuintaVela,"  De= ",Otras+SegundaVela+TerceraVela+CuartaVela+QuintaVela);
  }
//+------------------------------------------------------------------+
