...
input int Size_Max_Bottom=500;
input int Size_Min_Bottom=200;
input int Size_Body_Max=500;
input int Size_Body_Min=200;
input int Size_Max_Top=500;
   ...
   for(int shift=0; shift<end; shift++)
     {
      ...
      if(SizeBody>=Size_Body_Min*Point() && SizeBody<=Size_Body_Max*Point() && SizeBotton>=Size_Min_Bottom*Point() && SizeBotton<=Size_Max_Bottom*Point() && SizeTop<=Size_Max_Top*Point())
        {
         ArrowSignalDN[shift+1]=high[shift+1]+Gap_Arrow*Point();
         if(shift==0)
            Alertas("Alcista");
        }
