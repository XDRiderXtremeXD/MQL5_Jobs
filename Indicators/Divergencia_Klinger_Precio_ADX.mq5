//+------------------------------------------------------------------+
//|                               Divergencia_Klinger_Precio_ADX.mq5 |
...
int Puntos_Klinger,Puntos_Divergencia,ADX;

input string Settings_Indicator="***Settings Indicator***";
input int MaximoVelasADX=6;
...
datetime Sell_Tiempo=0,Buy_Tiempo=0;
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
int OnInit()
  {
   ...
   SetIndexBuffer(8,ADX_BASE,INDICATOR_CALCULATIONS);
   SetIndexBuffer(9,ADX_PLUS,INDICATOR_CALCULATIONS);
   SetIndexBuffer(10,ADX_MINUS,INDICATOR_CALCULATIONS);
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   ...
   if(BarsCalculated(ADX)<rates_total)
      return(0);
...
////// NAIN LINE............
   ...
   if(CopyBuffer(ADX,0,0,to_copy,ADX_BASE)<=0)
      return(0);
   if(CopyBuffer(ADX,1,0,to_copy,ADX_PLUS)<=0)
      return(0);
   if(CopyBuffer(ADX,2,0,to_copy,ADX_MINUS)<=0)
      return(0);
   ...
   for(int shift=Start ; shift<rates_total; shift++)
     {
               ...
               if(Condicion)
                 {
                  ...
                  Buy_Tiempo=time[shift]+PeriodSeconds((ENUM_TIMEFRAMES)Period())*MaximoVelasADX; 
      ...
      if(CoincidenciaSELL[shift-2]!=EMPTY_VALUE)
               ...
               if(Condicion)
                 {
                  ...
                  Sell_Tiempo=time[shift]+PeriodSeconds((ENUM_TIMEFRAMES)Period())*MaximoVelasADX; 
        ...
        if(Buy_Tiempo>time[shift] && Buy_Tiempo!=0 && ADX_BASE[shift-2]>ADX_PLUS[shift-2] && ADX_BASE[shift-1]<ADX_PLUS[shift-1]){
        ArrowSignalBuy[shift-1]=low[shift-1];
        Buy_Tiempo=0;
        } 
        if(Sell_Tiempo>time[shift] && Sell_Tiempo!=0 && ADX_BASE[shift-2]>ADX_MINUS[shift-2] && ADX_BASE[shift-1]<ADX_MINUS[shift-1]){
        ArrowSignalSell[shift-1]=high[shift-1];
        Sell_Tiempo=0;
        } 
     }
