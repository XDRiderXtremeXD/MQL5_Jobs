...
input string Settings_ADX_6="***Settings_ADX_6***";  ///*---
input uint    PeriodoADX_6=14;
...
int OnInit()
  {
   ...
   AwesomeOscilator=iAO(Symbol(),PERIOD_CURRENT);
      ADX_6=iADX(Symbol(),PERIOD_CURRENT,PeriodoADX_6);
   ...
   ArraySetAsSeries(AwesomeOscilator_Buffer,true);
    ArraySetAsSeries(ADX_6_Buffer,true);
...
//172, 115
   ...
   PlotIndexSetInteger(0,PLOT_ARROW,109);
   PlotIndexSetDouble(0,PLOT_EMPTY_VALUE,EMPTY_VALUE);

   PlotIndexSetInteger(1,PLOT_ARROW,109);
   ...
   for(int shift=Start; shift<rates_total; shift++)
     {
      ...
      CopyBuffer(Bollinger_1,1,rates_total-shift-1,3,Bollinger_1_Buffer_UP);
      CopyBuffer(Bollinger_1,2,rates_total-shift-1,3,Bollinger_1_Buffer_DN);
      CopyBuffer(Bollinger_1,0,rates_total-shift-1,3,Bollinger_1_Buffer_BASE);
      ...
      CopyBuffer(AwesomeOscilator,0,rates_total-shift-1,4,AwesomeOscilator_Buffer);
        CopyBuffer(ADX_6,0,rates_total-shift-1,5,ADX_6_Buffer);
      CopyBuffer(ADX_6,1,rates_total-shift-1,5,DI_6_plusBuffer);
      CopyBuffer(ADX_6,2,rates_total-shift-1,5,DI_6_minusBuffer);

      ResetLastError();
      
      if(Rates[1].close<Bollinger_1_Buffer_DN[1] &&
       ADXBuffer[0]>Bollinger_ADX_Buffer_UP[0] && Bollinger_ADX_Buffer_UP[0]<=Bollinger_ADX_Buffer_UP[1] && Bollinger_ADX_Buffer_UP[1]<Bollinger_ADX_Buffer_UP[2] && Bollinger_ADX_Buffer_UP[2]<Bollinger_ADX_Buffer_UP[3] && KlingerBuffer[0]<KlingerBuffer[1] && KlingerBuffer[1]<KlingerBuffer[2] && KlingerBuffer[2]<KlingerBuffer[3])
        {
         Buy_Tiempo=time[shift]+PeriodSeconds((ENUM_TIMEFRAMES)Period())*MaximoVelasKlinger;
        }
      ...
      ADXBuffer[0]>Bollinger_ADX_Buffer_UP[0] && Bollinger_ADX_Buffer_UP[0]<=Bollinger_ADX_Buffer_UP[1] && Bollinger_ADX_Buffer_UP[1]<Bollinger_ADX_Buffer_UP[2] && Bollinger_ADX_Buffer_UP[2]<Bollinger_ADX_Buffer_UP[3] && KlingerBuffer[0]>KlingerBuffer[1] && KlingerBuffer[1]>KlingerBuffer[2] && KlingerBuffer[2]>KlingerBuffer[3])
        ...
        {
         Sell_Tiempo=time[shift]+PeriodSeconds((ENUM_TIMEFRAMES)Period())*MaximoVelasKlinger;
        }

      if(Buy_Tiempo>time[shift] && Buy_Tiempo!=0 && KlingerBuffer[2]>Bollinger_Klinger_Buffer_BASE[2] && KlingerBuffer[3]<Bollinger_Klinger_Buffer_BASE[3]
       && KlingerBuffer[0]>KlingerBuffer[1]
      && ((DI_6_plusBuffer[2]<DI_6_minusBuffer[2]  && DI_6_plusBuffer[1]>DI_6_minusBuffer[1]  )   ||     ( DI_6_plusBuffer[2]<ADX_6_Buffer[2]  && DI_6_plusBuffer[1]>ADX_6_Buffer[1] ) ||     ( (MathAbs(DI_6_plusBuffer[1]-DI_6_minusBuffer[1])>22)  && DI_6_plusBuffer[1]>ADX_6_Buffer[1] ) ||     ( (MathAbs(DI_6_plusBuffer[1]-DI_6_minusBuffer[1])>23)  && ADX_6_Buffer[1]>DI_6_plusBuffer[1]   && DI_6_plusBuffer[1]>DI_6_minusBuffer[1] && ADX_6_Buffer[1]>DI_6_minusBuffer[1]  && ADX_6_Buffer[1]>26 ))
      ...
      if(Sell_Tiempo>time[shift] && Sell_Tiempo!=0 && KlingerBuffer[2]<Bollinger_Klinger_Buffer_BASE[2] && KlingerBuffer[3]>Bollinger_Klinger_Buffer_BASE[3]
       &&  KlingerBuffer[0]<KlingerBuffer[1]
       && ((DI_6_plusBuffer[2]>DI_6_minusBuffer[2]  && DI_6_plusBuffer[1]<DI_6_minusBuffer[1])  || ( DI_6_minusBuffer[2]>ADX_6_Buffer[2]  && DI_6_minusBuffer[1]<ADX_6_Buffer[1] )      ||     ( (MathAbs(DI_6_plusBuffer[1]-DI_6_minusBuffer[1])>22)  && DI_6_minusBuffer[1]>ADX_6_Buffer[1] )  ||     ( (MathAbs(DI_6_plusBuffer[1]-DI_6_minusBuffer[1])>23)  && ADX_6_Buffer[1]>DI_6_plusBuffer[1]   && DI_6_plusBuffer[1]<DI_6_minusBuffer[1] && ADX_6_Buffer[1]>DI_6_minusBuffer[1]  && ADX_6_Buffer[1]>26 )) 
