...
double crossUp[],crossDn[],cross[],ZERO_LAG[],DMIOS[];
//--- indicator handles
int _adxHandle;
int _atrHandle;
...
input string SEPARADOR2 = "-----ZERO-LAG--------";
...
//--- indicator buffers mapping
    ...
    SetIndexBuffer(3,ZERO_LAG,INDICATOR_CALCULATIONS);
   ...
   if(DefinitionZERO_LAG==INVALID_HANDLE) { return(INIT_FAILED); }
...
int OnCalculate(const int rates_total,const int prev_calculated,const datetime &time[],
                ...
                const int &spread[])
  {
   ...
   if(BarsCalculated(DefinitionZERO_LAG)<rates_total)  return(prev_calculated);
   ...
   {  
      ...
      int _ZeroLagCopied=CopyBuffer( DefinitionZERO_LAG,1,time[i],1,ZERO_LAG);
      ...
      { 
         if (cross[i]== 1 && ZERO_LAG[0]==2 && close[i]>open[i] && close[i-1]<open[i-1] && DMIOS[0]==1) { crossUp[i] =  low[i]-_atr[0]; Alarm=i==rates_total-1?true:false;}
         if (cross[i] == 2 && ZERO_LAG[0]==1 && close[i]<open[i] && close[i-1]>open[i-1] && DMIOS[0]==2) {crossDn[i] = high[i]+_atr[0]; Alarm=i==rates_total-1?true:false;}
      }
//---c
   }
