...
input int Periodo_Media_Slow=38;
...
input int MFI_Periodo=14;
input ENUM_MA_METHOD MFI_Mode=MODE_SMA;
input ENUM_APPLIED_VOLUME MFI_Volume_Type=VOLUME_TICK;
input string Settings_Bollinger_MFI="***Settings_Bollinger_MFI***";  ///*---
input int Bollinger_MFI_Periodo=20;
input int Bollinger_MFI_Shift=0;
input int Bollinger_MFI_Desviation=2;
...
input bool DireccionArraySimple4=false;//SetArraySeries Direccion
...
double MFIBuffer[];
double Bollinger_MFI_Buffer_DN[];
double Bollinger_MFI_Buffer_UP[];
double Bollinger_MFI_Buffer_BASE[];
...
int OnInit()
  {
   ...
   MFI=iMFI(Symbol(),PERIOD_CURRENT,MFI_Periodo,MFI_Volume_Type);
   Bollinger_MFI=iBands(Symbol(),PERIOD_CURRENT,Bollinger_MFI_Periodo,Bollinger_MFI_Shift,Bollinger_MFI_Desviation,MFI);
...
//172, 115
   ...
   ArraySetAsSeries(Bollinger_MFI_Buffer_DN,true);
   ArraySetAsSeries(Bollinger_MFI_Buffer_UP,true);
   ArraySetAsSeries(Bollinger_MFI_Buffer_BASE,true);
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   ...
   for(int shift=Start; shift<rates_total; shift++)
     {
      ...
      int copied=CopyRates(Symbol(),PERIOD_CURRENT,rates_total-1-shift,100,Rates);
      CopyBuffer(Bollinger_1,1,rates_total-1-shift,60,Bollinger_1_Buffer_UP);
      CopyBuffer(Bollinger_1,2,rates_total-1-shift,60,Bollinger_1_Buffer_DN);
      CopyBuffer(Bollinger_1,0,rates_total-1-shift,40,Bollinger_1_Buffer_Central);
      CopyBuffer(Bollinger_2,1,rates_total-1-shift,30,Bollinger_2_Buffer_UP);
      CopyBuffer(Bollinger_2,2,rates_total-1-shift,30,Bollinger_2_Buffer_DN);
      ...
      CopyBuffer(MFI,0,rates_total-shift-1,4,MFIBuffer);
      CopyBuffer(Bollinger_MFI,2,rates_total-shift-1,4,Bollinger_MFI_Buffer_DN);
      CopyBuffer(Bollinger_MFI,1,rates_total-shift-1,4,Bollinger_MFI_Buffer_UP);
    ...
    if(  (ROC_Buffer[2]>Bollinger_ROC_Buffer_BASE[2]) && (ROC_Buffer[1]<Bollinger_ROC_Buffer_BASE[1]) &&  (MathAbs(Media_Fast_Buffer[5]-Media_Slow_Buffer[5]))<(MathAbs(Media_Fast_Buffer[4]-Media_Slow_Buffer[4])) &&  (MathAbs(Media_Fast_Buffer[4]-Media_Slow_Buffer[4]))<(MathAbs(Media_Fast_Buffer[3]-Media_Slow_Buffer[3])) &&  (MathAbs(Media_Fast_Buffer[6]-Media_Slow_Buffer[6]))<(MathAbs(Media_Fast_Buffer[5]-Media_Slow_Buffer[5]))
  )
   ...
   if((ROC_Buffer[2]<Bollinger_ROC_Buffer_BASE[2]) && (ROC_Buffer[1]>Bollinger_ROC_Buffer_BASE[1])  &&  (MathAbs(Media_Fast_Buffer[5]-Media_Slow_Buffer[5]))<(MathAbs(Media_Fast_Buffer[4]-Media_Slow_Buffer[4])) &&  (MathAbs(Media_Fast_Buffer[4]-Media_Slow_Buffer[4]))<(MathAbs(Media_Fast_Buffer[3]-Media_Slow_Buffer[3])) &&  (MathAbs(Media_Fast_Buffer[6]-Media_Slow_Buffer[6]))<(MathAbs(Media_Fast_Buffer[5]-Media_Slow_Buffer[5]))
       ...
       
        {
         ...
         if(Buy_Tiempo_ROC>time[shift] &&  Buy_Tiempo_ROC!=0 
         && (Rates[6].close<Rates[6].open && Rates[4].close>Rates[4].open && ((Rates[3].close>Rates[3].open) || (Rates[2].close>Rates[2].open)) || (Rates[6].close>Rates[6].open && Rates[4].close<Rates[4].open && ((Rates[3].close<Rates[3].open) || (Rates[2].close<Rates[2].open))))
         && ROC_Buffer[2]<Bollinger_ROC_Buffer_BASE[2] && ROC_Buffer[1]>Bollinger_ROC_Buffer_BASE[1] && SARBuffer[2]>=Rates[2].high && SARBuffer[1]<=Rates[1].low  && ROC_Buffer[1]<0 && ROC_Buffer[0]>=0
         
         ) {
         MarkPattern(GetName("ROC",time[shift-1]), time[shift-1], low[shift-1] - (Pointer_Offset+Offset_Subida_VelaOpuesta+CumOffset)*_Point, "ROC", Text_VelasOpuestas, COLORVO);
         ...
         if(Sell_Tiempo_ROC>time[shift]  && Sell_Tiempo_ROC!=0 
         && (Rates[6].close<Rates[6].open && Rates[4].close>Rates[4].open && ((Rates[3].close>Rates[3].open) || (Rates[2].close>Rates[2].open)) || (Rates[6].close>Rates[6].open && Rates[4].close<Rates[4].open && ((Rates[3].close<Rates[3].open) || (Rates[2].close<Rates[2].open))))
         &&  ROC_Buffer[2]>Bollinger_ROC_Buffer_BASE[2] && ROC_Buffer[1]<Bollinger_ROC_Buffer_BASE[1] && SARBuffer[1]>=Rates[1].high && SARBuffer[2]<=Rates[2].low  && ROC_Buffer[1]>0 && ROC_Buffer[0]<=0
            
         ) {
        MarkPattern(GetName("ROC",time[shift-1]), time[shift-1], high[shift-1] + (Pointer_Offset+Offset_Caida_VelaOpuesta+High_Offset+CumOffset)*_Point,"ROC", Text_VelasOpuestas, COLORVO);
         CumOffset = CumOffset+IncOffset;
  ...
  if(Divergencia_Puntos_1_Buffer_DN[2]!=EMPTY_VALUE && Divergencia_Puntos_2_Buffer_DN[2]!=EMPTY_VALUE && Divergencia_Puntos_3_Buffer_DN[2]!=EMPTY_VALUE && Divergencia_Puntos_4_Buffer_DN[2]!=EMPTY_VALUE && Bollinger_2_Buffer_DN[2]<Bollinger_1_Buffer_DN[2] &&  Rates[1].close>Rates[2].close && Rates[2].close>Rates[3].close &&  CCIBuffer[0]>CCIBuffer[1] && CCIBuffer[1]>CCIBuffer[2])
      ...
      if(Buy_Tiempo_ADX2>time[shift] && Buy_Tiempo_ADX2!=0 && KlingerBuffer[2]>Bollinger_Klinger_Buffer_BASE[2] && KlingerBuffer[3]<Bollinger_Klinger_Buffer_BASE[3]
         ...
         || ((MathAbs(DI_6_plusBuffer[1]-DI_6_minusBuffer[1])>22)  && DI_6_plusBuffer[1]>ADX_4_Buffer[1]) || ((MathAbs(DI_6_plusBuffer[1]-DI_6_minusBuffer[1])>23)  && ADX_4_Buffer[1]>DI_6_plusBuffer[1]   && DI_6_plusBuffer[1]>DI_6_minusBuffer[1] && ADX_4_Buffer[1]>DI_6_minusBuffer[1]  && ADX_4_Buffer[1]>26))
         && (Rates[6].close<Rates[6].open && Rates[4].close>Rates[4].open && ((Rates[3].close>Rates[3].open) || (Rates[2].close>Rates[2].open)) || (Rates[6].close>Rates[6].open && Rates[4].close<Rates[4].open && ((Rates[3].close<Rates[3].open) || (Rates[2].close<Rates[2].open))))
         )
        {
     ...
     {
         MarkPattern(GetName("CRUCE ADX 11 4",time[shift-1]), time[shift-1], low[shift-1] - (Pointer_Offset+Offset_Subida_VelaOpuesta+CumOffset)*_Point, "C_ADX_114", Text_VelasOpuestas, COLORVO);
         ArrowSignalDN[shift-1] = low[shift-1] - (Pointer_Offset*_Point);
         if(shift==rates_total-1)
       {
         if(AlertPC)
     ...
     {
         MarkPattern(GetName("CRUCE ADX 11 4",time[shift-1]), time[shift-1], high[shift-1] + (Pointer_Offset+Offset_Caida_VelaOpuesta+High_Offset+CumOffset)*_Point,"C_ADX_114", Text_VelasOpuestas, COLORVO);
         ArrowSignalUP[shift-1] = high[shift-1] + (Pointer_Offset*_Point);
         if(shift==rates_total-1)
     
     {
      ...
      if(Buy_Tiempo_ADX3_Inv>time[shift] 
       && (Rates[6].close<Rates[6].open && Rates[4].close>Rates[4].open && ((Rates[3].close>Rates[3].open) || (Rates[2].close>Rates[2].open)) || (Rates[6].close>Rates[6].open && Rates[4].close<Rates[4].open && ((Rates[3].close<Rates[3].open) || (Rates[2].close<Rates[2].open))))
      && Buy_Tiempo_ADX3_Inv!=0 && KlingerBuffer[2]>Bollinger_Klinger_Buffer_BASE[2] && KlingerBuffer[3]<Bollinger_Klinger_Buffer_BASE[3]
      ...
      if(Sell_Tiempo_ADX3_Inv>time[shift] 
          && (Rates[6].close<Rates[6].open && Rates[4].close>Rates[4].open && ((Rates[3].close>Rates[3].open) || (Rates[2].close>Rates[2].open)) || (Rates[6].close>Rates[6].open && Rates[4].close<Rates[4].open && ((Rates[3].close<Rates[3].open) || (Rates[2].close<Rates[2].open))))   
         && Sell_Tiempo_ADX3_Inv!=0 && KlingerBuffer[2]<Bollinger_Klinger_Buffer_BASE[2] && KlingerBuffer[3]>Bollinger_Klinger_Buffer_BASE[3]
         &&  KlingerBuffer[0]<KlingerBuffer[1]
         && ((DI_6_plusBuffer[2]>DI_6_minusBuffer[2]  && DI_6_plusBuffer[1]<DI_6_minusBuffer[1])  || (DI_6_minusBuffer[2]>ADX_4_Buffer[2]  && DI_6_minusBuffer[1]<ADX_4_Buffer[1])
         || ((MathAbs(DI_6_plusBuffer[1]-DI_6_minusBuffer[1])>22)  && DI_6_minusBuffer[1]>ADX_4_Buffer[1])  || ((MathAbs(DI_6_plusBuffer[1]-DI_6_minusBuffer[1])>23) 
         && ADX_4_Buffer[1]>DI_6_plusBuffer[1]   && DI_6_plusBuffer[1]<DI_6_minusBuffer[1] && ADX_4_Buffer[1]>DI_6_minusBuffer[1]  && ADX_4_Buffer[1]>26))
      ...
      && (Rates[6].close<Rates[6].open && Rates[4].close>Rates[4].open && ((Rates[3].close>Rates[3].open) || (Rates[2].close>Rates[2].open)) || (Rates[6].close>Rates[6].open && Rates[4].close<Rates[4].open && ((Rates[3].close<Rates[3].open) || (Rates[2].close<Rates[2].open))))
...

        {
         ...
         for(int j=1; j<36; j++)
           {
            if((DI_4_minusBuffer_Dif[j]>20 && DI_3_minusBuffer_Dif[j]>22 && DI_2_minusBuffer_Dif[j]>25 && DI_minusBuffer_Dif[j]>27) && (DI_4_minusBuffer_Dif[j+1]>20 && DI_3_minusBuffer_Dif[j+1]>22 && DI_2_minusBuffer_Dif[j+1]>25 && DI_minusBuffer_Dif[j+1]>27)  && (DI_4_minusBuffer_Dif[j+2]>20 && DI_3_minusBuffer_Dif[j+2]>22 && DI_2_minusBuffer_Dif[j+2]>25 && DI_minusBuffer_Dif[j+2]>27)         && (DI_4_minusBuffer_Dif[j+3]>20 && DI_3_minusBuffer_Dif[j+3]>22 && DI_2_minusBuffer_Dif[j+3]>25 && DI_minusBuffer_Dif[j+3]>25)
            )
              {
               AlertaCondicion1=false;
               break;
