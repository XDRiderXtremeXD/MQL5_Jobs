...
#property indicator_label9  "Filling"
...
input string comment_1c = "==========";   // BackgroundFill Settings
input bool backgroundFill=true;
input color colorStrong=clrDarkGreen;
input color colorWeak=clrRed;
input int rangeFill=25;
...
string PairMajor="USD";
string PairSecondary="USD";
...
int OnInit()
  {
   ...
   for(int i = 0; i < ArraySize(AllCurrencies); i++)
     {
      ...
      int indexFindPair=StringFind(Symbol(), AllCurrencies[i], 0);

      if((indexFindPair >= 0) || (DrawAllCurrencies))
        {
         if(indexFindPair==0)
            PairMajor=AllCurrencies[i];
         else
            PairSecondary=AllCurrencies[i];
      ...
      if(ErrorLog)
         Print("Created index ", i, " for ", AllCurrencies[i]);
     }

   SetIndexBuffer(8,FILLIN_UP,INDICATOR_DATA);
   SetIndexBuffer(9,FILLIN_DN,INDICATOR_DATA);
   ...
   ArraySetAsSeries(FILLIN_UP, true);
   ArraySetAsSeries(FILLIN_DN, true);
   ...
   ArrayInitialize(FILLIN_UP, EMPTY_VALUE);
   ArrayInitialize(FILLIN_DN, EMPTY_VALUE);

   PlotIndexSetInteger(8,PLOT_LINE_COLOR,0,colorStrong);
   PlotIndexSetInteger(8,PLOT_LINE_COLOR,1,colorWeak);
...
void OnTimer()
  {
   ...
   int rsi_limit = limit;
   if(rsi_limit < SmoothingPeriod)
      rsi_limit = limit + SmoothingPeriod;
   if(!GetRSIValue(rsi_limit))
      HistoricalOK = false;
   if(HistoricalOK)
     {
      bool calc_success = CalculateBuffers(limit); // Did some pairs failed due to bars' desync?
      if(!calc_success)
         return; // Recalculate next time.
     }
   if(!HistoricalOK)
     {
      if(DrawPanel)
   ...
   if(limit == iBars(Symbol(), Period()))
      limit--; // Avoid array out of range.
   if(ShowSignals)
      DrawArrows(limit);
  }
...
int OnCalculate(const int rates_total,
                ...
                const int& spread[])
  {
   ...
   if((LimitBars) && (limit > MaxBars))
      limit = MaxBars;
   if(rates_total < limit + RSIPeriod)
      limit = rates_total;
   int rsi_limit = limit;
   if(rsi_limit < SmoothingPeriod)
      rsi_limit = limit + SmoothingPeriod;
   ...
   if(HistoricalOK)
     {
      bool calc_success = CalculateBuffers(limit); // Did some pairs failed due to bars' desync?
      if(!calc_success)
         return prev_calculated; // Recalculate next time.
     }
   if(!HistoricalOK)
     {
      if(DrawPanel)
      ...
      if(id == CHARTEVENT_CHART_CHANGE)
        {
         ...
         if(ShowSignals)
            DrawArrows(Bars(Symbol(), Period()) - 1);
        }
  }
...
string CalculationModeDesc()
  {
   string Text = "";
   if(CalculationMode == Mode_ASITot)
      Text = "ASI TOT";
   if(CalculationMode == Mode_ASITotMA)
      Text = "ASI TOT MA";
   if(CalculationMode == Mode_ROC)
      Text = "ROC TOT";
   if(CalculationMode == Mode_ROCMA)
      Text = "ROC TOT MA";
   return Text;
  }
...
bool SetAllPairs()
  {
   if((CurrPrefix == "") && (CurrSuffix == ""))
      return true;
   if(ErrorLog)
      Print("Found Prefix = ", CurrPrefix, " and Suffix = ", CurrSuffix);
   ...
   for(int i = 0; i < ArraySize(AllPairs); i++)
     {
      ...
      if(ErrorLog)
         Print("RSI handle for ", AllPairs[i], " = ", RSIHandle[i]);
      if(RSIHandle[i] == INVALID_HANDLE)
        {
         if(ErrorLog)
            Print("Handle initialization failed for ", AllPairs[i]);
         return false;
        }
     }
   return true;
   ...
   for(int i = 0; i < ArraySize(AllPairs); i++)
     {
      ...
      if(ErrorLog)
         Print("Copied for ", AllPairs[i], " ", c, " values");
      if(c < 0)
        {
         if(ErrorLog)
            Print("Error copying ", AllPairs[i], " data - ", GetLastErrorText(GetLastError()), " - ", GetLastError(), " - index: ", i);
         return false;
        }
      if(c < Max)
        {
         if(ErrorLog)
            Print("Not enough values for ", AllPairs[i], " data - ", GetLastErrorText(GetLastError()), " - ", GetLastError(), " - index: ", i, " found only ", c);
         HistoricalOK = false;
      ...
      for(int j = 0; j < ArraySize(RSIValueTemp); j++)
        {
         if((RSIValueTemp[j] == NULL) || (RSIValueTemp[j] == EMPTY_VALUE))
           {
            if(ErrorLog)
               Print("Value not valid for for ", AllPairs[i], " value - ", RSIValueTemp[j], " - shift ", j, " - index: ", i);
            HistoricalOK = false;
            MissingHistoricalPair = AllPairs[i];
            return false;
           }
...
void Fill(int i)
  {
   double pairMayor=GetValueBufferPair(PairMajor,i);
   double pairSecondary=GetValueBufferPair(PairSecondary,i);

   if(pairMayor>pairSecondary)
     {
      FILLIN_DN[i]=-rangeFill;
      FILLIN_UP[i]=rangeFill;
     }
   else
     {
      FILLIN_DN[i]=rangeFill;
...
//|                                                                  |
//+------------------------------------------------------------------+
double GetValueBufferPair(string pair,int index)
  {
   if(pair=="EUR")
      return EUR[index];
   if(pair== "USD")
...
void DrawMissingHistorical()
  {
   ...
   if(StringLen(MissingHistoricalPair) == 0)
      ObjectSetString(0, MissingHistoricalGoTo, OBJPROP_TEXT, "LOADING");
   else
...
double GetROCStrength(string Curr, int j)
  {
   ...
   for(int i = 0; i < ArraySize(AllPairs); i++)
     {
      if(StringFind(AllPairs[i], Curr, 0) < 0)
         continue;
      if(iTime(AllPairs[i], LinesTF, 0) != iTime(_Symbol, LinesTF, 0))
         return EMPTY_VALUE; // A new bar on the currency pair didn't update yet. Assuming that current symbol is always up-to-date.
      int k = j;
      if(LinesTF != Period())
         k = iBarShift(AllPairs[i], LinesTF, iTime(AllPairs[i], Period(), j), false);
      ...
      if((StartValue == 0) || (EndValue == 0) || (k == -1) || (k > Bars(AllPairs[i], LinesTF)) || (iTime(AllPairs[i], Period(), j) == 0))
        {
         if(ErrorLog)
            Print("Value not valid for ", AllPairs[i], " value - ", k);
         HistoricalOK = false;
         MissingHistoricalPair = AllPairs[i];
         return 0;
        }
      if(ErrorLog)
         Print("Using ROC value of ", SValue, " for ", AllPairs[i], " and shift ", j);
   ...
   if(ErrorLog)
      Print("Calculated ROC strength for ", Curr, " and shift ", j, " with a value of ", Tot);
...
double GetRSIStrength(string Curr, int j)
  {
   ...
   for(int i = 0; i < ArraySize(AllPairs); i++)
     {
      ...
      if(ErrorLog)
         Print("Using RSI value of ", SValue, " for ", AllPairs[i], " and shift ", j);
      if(StringFind(AllPairs[i], Curr, 0) < 3)
        {
   ...
   if(ErrorLog)
      Print("Calculated RSI strength for ", Curr, " and shift ", j, " with a value of ", Tot);
   return NormalizeDouble(Tot, IndicatorDigits);
  }
...
void CopyBaseQuote()
  {
   if(ErrorLog)
      Print("Copy Base ", CurrBase, " and Quote ", CurrQuote);
...
int DrawArrow(int i)
  {
   ...
   bool Sell = false;
   bool Neutral = false;
   if((ArraySize(Base) == 0) || (ArraySize(Quote) == 0))
      return LastArrow;
   if(i >= ArraySize(Base) - 1)
      return LastArrow;
   ValueBaseCurr = Base[i];
   ValueBasePrev = Base[i + 1];
   ValueQuoteCurr = Quote[i];
   ValueQuotePrev = Quote[i + 1];
   if((ValueBaseCurr == EMPTY_VALUE) || (ValueBasePrev == EMPTY_VALUE) || (ValueQuoteCurr == EMPTY_VALUE) || (ValueQuotePrev == EMPTY_VALUE))
      return LastArrow;

   if((ValueBasePrev < ValueQuotePrev) && (ValueBaseCurr > ValueQuoteCurr) && (AboveBelow) && (!OppositeZeros))
      Buy = true;
   if((ValueBasePrev > ValueQuotePrev) && (ValueBaseCurr < ValueQuoteCurr) && (AboveBelow) && (!OppositeZeros))
      Sell = true;
   if((ValueBaseCurr > 0) && (ValueQuoteCurr < 0) && ((ValueQuotePrev > 0) || (ValueBasePrev < 0)) && (OppositeZeros))
      Buy = true;
   if((ValueBaseCurr < 0) && (ValueQuoteCurr > 0) && ((ValueQuotePrev < 0) || (ValueBasePrev > 0)) && (OppositeZeros))
      Sell = true;
...
void NotifyArrow()
  {
   ...
   if((LastAlertDirection == ArrowDesc) && (LastNotification == ArrowDate))
      return; // Same arrow, don't alert again.
   string Setup = "";
   if(ArrowDesc == "SELL")
      Setup = "Possible SELL setup";
   else
      if(ArrowDesc == "BUY")
         Setup = "Possible BUY setup";
      else
         if(ArrowDesc == "NEUTRAL")
            Setup = "Possible STOP setup";
         else
            return;
