   ...
   if(Divergencia_Puntos_1_Buffer_UP[2]!=EMPTY_VALUE && Divergencia_Puntos_2_Buffer_UP[2]!=EMPTY_VALUE && Rates[5].high<EMA_Buffer_Alligator[5] &&  
      ...
      && Rates[1].close<Bollinger_1_Buffer_UP[1] && EMA_Buffer_Alligator[2]>Bollinger_1_Buffer_UP[2])
     {
      ...
      Sell_Tiempo=time[shift]+PeriodSeconds((ENUM_TIMEFRAMES)Period())*MaximoVelasSar;
         ...
         if(Buy_Tiempo>time[shift] && SARBuffer[2]>=Rates[2].high && SARBuffer[1]<=Rates[1].low && Buy_Tiempo!=0 && Bollinger_1_Buffer_Central[0]<Rates[0].close){
         ...
         if(Sell_Tiempo>time[shift] && SARBuffer[1]>=Rates[1].high && SARBuffer[2]<=Rates[2].low && Sell_Tiempo!=0 && Bollinger_1_Buffer_Central[0]>Rates[0].close){
         CumOffset = CumOffset+IncOffset;
