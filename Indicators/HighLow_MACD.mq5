...
//---
   ...
   for(int shift=MathMin(rates_total-2,to_copy-1); shift>=0; shift--)
     {
...
//--- try to copy
   ...
   if(CopyBuffer(macd_handle,1,0,toCopy,MACD_COLOR)<=0)
      return(false);
   if(CopyBuffer(HL_handle,0,0,toCopy,HIGH)<=0)
      return(false);
   if(CopyBuffer(HL_handle,1,0,toCopy,LOW)<=0)
      return(false);
...
bool CondicionEntrada(bool &EsCompra,int shift,double const &High[],double const &Low[])
  {
   if(MACD_COLOR[shift+1]==0 && (HIGH[shift+1]-puntos_cercania*Point())<=High[shift])
     {
      EsCompra=false;
      return true;
     }
   if(MACD_COLOR[shift+1]==1 && (LOW[shift+1]+puntos_cercania*Point())>=Low[shift])
     {
      EsCompra=true;
      return true;
     }
