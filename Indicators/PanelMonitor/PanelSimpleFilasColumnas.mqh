...
public:
   ...
   virtual void       InicializarArrayDiferente() {ArrayResize(DifSizeAncho,0); ArrayResize(DifSizeLargo,0);};
...
void   Panelfilas_columnas_::Text(int columna,int fila,string Texto)
  {
   int point_c=right?(MathAbs(columna-(columnas-1))):columna;
   int point_f=low?(MathAbs(fila-(filas-1))):fila;
