//+------------------------------------------------------------------+
//|                                   ADX_3_Cruce_Medias_Moviles.mq5 |
...
input int MaximoVelasCruceMedias=12;
input string Settings_Media_Movil_Slow="***Settings Media Movil Slow***"; ///*---
input int Periodo_Media_Slow=7;
   ...
   for(int shift=Start; shift<rates_total; shift++)
     {
      ...
      CopyBuffer(MediaFast,0,rates_total-shift-1,3,Media_Fast_Buffer);
      CopyBuffer(MediaSlow,0,rates_total-shift-1,3,Media_Slow_Buffer);
      ...
      if(Rates[1].close>Bollinger_1_Buffer_UP[1]  &&
        ...
        {
         Buy_Tiempo=time[shift]+PeriodSeconds((ENUM_TIMEFRAMES)Period())*(MaximoVelasCruceMedias+1);
        }

      if(Rates[1].close<Bollinger_1_Buffer_DN[1] &&
         ADXBuffer[0]>Bollinger_ADX_Buffer_UP[0] && Bollinger_ADX_Buffer_UP[0]<=Bollinger_ADX_Buffer_UP[1] && Bollinger_ADX_Buffer_UP[1]<Bollinger_ADX_Buffer_UP[2] && Bollinger_ADX_Buffer_UP[2]<Bollinger_ADX_Buffer_UP[3] && KlingerBuffer[0]<KlingerBuffer[1] && KlingerBuffer[1]<KlingerBuffer[2] && KlingerBuffer[2]<KlingerBuffer[3])
        {
         Sell_Tiempo=time[shift]+PeriodSeconds((ENUM_TIMEFRAMES)Period())*(MaximoVelasCruceMedias+1);
        }

      if(Buy_Tiempo>time[shift] && Buy_Tiempo!=0 && Media_Fast_Buffer[2]<Media_Slow_Buffer[2] && Media_Fast_Buffer[1]>Media_Slow_Buffer[1])
        {
      ...
      if(Sell_Tiempo>time[shift] && Sell_Tiempo!=0 && Media_Fast_Buffer[2]>Media_Slow_Buffer[2] && Media_Fast_Buffer[1]<Media_Slow_Buffer[1])
        {
